...
// Lease estimate (fractional stays, smooth)
function leaseEstimateFractional({neigh,unit,ops,targetMarginPct,includeFixed}){
  const base=baselineFrom(neigh,unit); const roll=opsRollup(ops);
  ...
  const fixed=DRP_DATA.fixed_expenses_defaults; const fixedMonthly=includeFixed?(fixed.internet+fixed.insurance+fixed.hoa):0;
  ...
  return {gross,staysFrac,variable,fixedMonthly,marginMAD,rentCap,suggested, avgStay: base.avg_stay_nights};
}
...
function wireLease(){navActive(); const f=$('#calc-form'); $('#ops-host',f).innerHTML=opsControlsHtml();
  function run(){const neigh=$('#neigh').value, unit=$('#unit').value; const ops=readOps(f);
    const res=leaseEstimateFractional({neigh,unit,ops,targetMarginPct:DRP_DATA.lease_target_margin_pct,includeFixed:true});
    $('#headline-val').textContent=fmt(res.suggested); $('#d-gross').textContent=fmt(res.gross);
    $('#d-stays').textContent=(Math.round(res.staysFrac*10)/10).toFixed(1);
    $('#d-variable').textContent=fmt(res.variable); $('#d-fixed').textContent=fmt(res.fixedMonthly);
    $('#d-margin').textContent=fmt(res.marginMAD); $('#d-rentcap').textContent=fmt(res.rentCap); $('#d-avgstay').textContent=res.avgStay.toFixed(1);}
  bind(f,run); $('#calc')?.addEventListener('click',run); run();}
